...
//[ej]
int FATINFO_bootSec;
int FATINFO_reservedSec;
int FATINFO_rootDirClus;
int FATINFO_bytePerSec;
int FATINFO_secPerClus;
int FATINFO_secPerFAT;
int FATINFO_filesysFAT;
bool FATINFO_mirroring;
...
//[ej]
u32 FAT_getFileEntry(u32 cluster);
...
u32 FAT_LinkFreeCluster(u32 cluster)
{
	...
	// Get a free cluster
	///iprintf("fatFirstFree = %d\n",fatFirstFree); //[will:debug]
	
	firstFree = FAT_FirstFreeCluster();
	
	//iprintf("firstFree = %d\n",firstFree); //[will:debug]
...
//	iprintf(" ff=%x", firstFree);//[ej:debug]
...
bool FAT_InitFiles (void)
{
	...
	//[ej:disable mirroring]
	if (!(bootSec->extBlock.fat32.extFlags & 0x8000))
	{
		bootSec->extBlock.fat32.extFlags |= 0x8000;
		disc_WriteSector (bootSector,  bootSec);
	}
	...
	FATINFO_bytePerSec = filesysBytePerSec;
	FATINFO_secPerClus = filesysSecPerClus;
	FATINFO_reservedSec = bootSec->reservedSectors;
	FATINFO_secPerFAT = filesysSecPerFAT;
	...
	else	// Set up for the FAT32 way
	{
		...
		{
			// Use the active FAT
			FATINFO_mirroring = true;
			filesysFAT = filesysFAT + ( filesysSecPerFAT * (bootSec->extBlock.fat32.extFlags & 0x000F));
		}
		else
			FATINFO_mirroring = false;
	}
	FATINFO_rootDirClus = filesysRootDirClus;
	FATINFO_filesysFAT = filesysFAT;
	...
	do {
		...
		if (wrkDirOffset == BYTE_PER_READ / sizeof (DIR_ENT))
		{
			...
			if ((wrkDirSector == filesysSecPerClus) && (wrkDirCluster != FAT16_ROOT_DIR_CLUSTER))
			{
				...
				{
//					iprintf(" eof!"); //[ej:debug]
					notFound = true;
					break;	//[ej]
				}
//				iprintf(" Clust:%x", wrkDirCluster);//[ej:debug]
...
u32 FAT_GetFileSize (void)
{
	// Read in the last accessed directory entry
	disc_ReadSector ((wrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(wrkDirCluster)) + wrkDirSector, globalBuffer);
//	iprintf(" clst_qf:%x,off:%d", wrkDirCluster, wrkDirOffset); //[ej:debug]
...
DIR_ENT FAT_DirEntFromPath (const char* path)
{
	...
	Tunicode PathUnicode[MAX_FILENAME_LENGTH];
	...
	while (!notFound && !found)
	{
		...
		if ((path[pathPos] == '.') && ((path[pathPos + 1] == '\0') || (path[pathPos + 1] == '/'))) {
			...
			dotSeen = false;
			while ((namePos < MAX_FILENAME_LENGTH - 1) && (path[pathPos] != '\0') && 
				(path[pathPos] != '/'))
			{
				//name[namePos] = ucase(path[pathPos]);
				#if 1
				...
				u32 len=0;
				// parse UTF-8 string
					if (path[pathPos] & 0x80)
					{
						if (path[pathPos] & 0x40)
						{
							if (path[pathPos] & 0x20)
							{
								{
									// 3 byte: 1110xxxx 10xxxxxx 10xxxxxx
									*ucode = ((path[pathPos] & 0x0F) << 12) | 
											((path[pathPos+1] & 0x3F) << 6) | 
											(path[pathPos+2] & 0x3F);
									len = 3;
								}
							}
							else
							{
								...
								*ucode = ((path[pathPos] & 0x1F) << 6) | 
										(path[pathPos+1] & 0x3F);	
								len = 2;
							}
							flagLFN = true;
							name[namePos] = *ucode & 0xff;
							PathUnicode[namePos] = *ucode & 0xffff;
							//name[namePos+1] = (*ucode >> 8) & 0xff;
					...
					else
					{
						// 1 byte: 0xxxxxxx
						name[namePos] = ucase(path[pathPos]);
						PathUnicode[namePos] = (path[pathPos]) & 0xff;
						len=1;
					}
				#endif
				if ((name[namePos] <= ' ') || 
					((name[namePos] >= ':') && (name[namePos] <= '?'))) // Invalid character
				{
					flagLFN = true;
				}
				if (name[namePos] == '.') {
		...
		while ( !found && !notFound)
		{
			...
			found = true;
				...
				{
					for (namePos = 0; (namePos < MAX_FILENAME_LENGTH) && 
						found && (name[namePos] != '\0') && (lfnName[namePos] != '\0'); namePos++)
					{
						if (name[namePos] != ucase(lfnName[namePos]))
						{
							found = false;
			...
			if(lfnExists)
				...
				found = true;
					for (namePos = 0; (namePos < MAX_FILENAME_LENGTH) && 
						found && (PathUnicode[namePos] != '\0') && 
						(lfnNameUnicode[namePos] != '\0'); 
						namePos++)
					{
						if (PathUnicode[namePos] != lfnNameUnicode[namePos])
						{
							found = false;
						}
					}
					if ((PathUnicode[namePos] == '\0') != (lfnNameUnicode[namePos] == '\0'))
					{
						found = false;
					}
				}
			...
			if (!found)
			{
				FAT_GetFilename(dirEntry, alias);
				found = true;
				for (namePos = 0; (namePos < 13) && found && 
					(name[namePos] != '\0') && (alias[namePos] != '\0'); namePos++)
				{
					if (name[namePos] != ucase(alias[namePos]))
					{
						found = false;
...
//	iprintf("path %s\n",&path[pathPos]); //[will:debug]
	...
	// Add end of string char
	//filename[filePos++] = '\0';   //will annotation
	...
	//iprintf("filesysRootDir %d\n",filesysRootDir);
	//iprintf("curWorkDirCluster %x\n",curWorkDirCluster); //[will:debug]
	//iprintf("filesysSecPerClus %d\n",filesysSecPerClus);
	dirEntryRemain = dirEntryLength;
	tempDirCluster = dirCluster;
	tempSecOffset = secOffset;    //0
	tempEntryOffset = entryOffset;  //0
	//iprintf("dirEntries %c\n",dirEntries[entryOffset].name[0]);	
	...
	u32 lastsecoffset=secOffset;
	u32 lastfirstSector=firstSector;
	...
	{		
		if ((dirEntries[entryOffset].name[0] == FILE_FREE) || (dirEntries[entryOffset].name[0] == FILE_LAST) )
		{
			...
			tempEntryOffset = entryOffset;  //14
		}
		...
		{
			lastsecoffset = secOffset;
			lastfirstSector = firstSector;
	...
	iprintf("dirEndFlag %d\n",dirEndFlag); //[will:debug]
	
	iprintf("firstSector %d\n",firstSector); //[will:debug]
	iprintf("lastfirstSector %d\n",lastfirstSector); //[will:debug]
	
	iprintf("lastsecoffset %d\n",lastsecoffset); //[will:debug]
	
	iprintf("secOffset %d\n",secOffset); //[will:debug]
	
	iprintf("entryOffset %d\n",entryOffset); //[will:debug]
	...
	if (dirEndFlag && (entryOffset == 15))	{		//entryOffset == 0
		...
		if(entryOffset == 0)
			...
			disc_ReadSector (lastfirstSector + secOffset, dirEntries);
	}

	iprintf("entryOffset %d\n",entryOffset); //[will:debug]

	//iprintf("dirEntryRemain = %d\n",dirEntryRemain); //[will:debug]
	...
	while (dirEntryRemain > 0)	
	{
		...
		if (entryOffset == BYTE_PER_READ / sizeof (DIR_ENT))
		{
			...
			if (dirEndFlag)	{
				memset (dirEntries, FILE_LAST, BYTE_PER_READ);
//				iprintf("memset!(%x %x %x)", dirCluster, entryOffset, secOffset); //[ej:debug]
			} else {
				disc_ReadSector (firstSector + secOffset, dirEntries);
			}
		}
		...
		else
		{
			...
			if(dirEndFlag)
				{
					if(entryOffset < ((BYTE_PER_READ / sizeof (DIR_ENT))-1))
						dirEntries[entryOffset+1].name[0] = FILE_LAST;
					//else
						//dirEntries[0].name[0] = FILE_LAST;
				}
	...
	// Write directory back to disk  (2 0)
	if(entryOffset == 0)
		disc_WriteSector (firstSector + secOffset, dirEntries);
	else
		disc_WriteSector (lastfirstSector + lastsecoffset, dirEntries);
...
FILE_TYPE FAT_FindNextFile(char* filename)
{
	...
	//[ej:debug]iprintf(" %x", file.name[0]);
...
//			iprintf("%s not exists!\n", path); //[ej:debug]
	...
	if ( strpbrk(mode, "wW") != NULL ) // (ucase(mode[0]) == 'W')
	{
		...
		//iprintf("fatFirstFree = %x\n",fatFirstFree); //[will:debug]
		//iprintf("curWorkDirCluster = %x\n",curWorkDirCluster); //[will:debug]
		startCluster = FAT_LinkFreeCluster (CLUSTER_FREE);
		//iprintf("startCluster = %x\n",startCluster); //[will:debug]
		//iprintf("fatFirstFree = %x\n",fatFirstFree); //[will:debug]
...
bool FAT_freopen(FAT_FILE* file)
{
  // Clear memory used by file information
  if ((file != NULL) && (file->inUse == true))
  {
    ...
    {
      if(file->length==0){
        iprintf("Fatal error: FileSystem: Not support 0byte file writed.\n");
...
bool FAT_SetSize(FAT_FILE *file, const u32 size, u8 FillChar)
{
  if(file->length!=0){
    iprintf("Fatal error: FAT2_SetSize already file size is not zero.\n");
    return 0;//ShowLogHalt();
  }
  
  if(file->write==false){
    iprintf("Fatal error: FAT2_SetSize file handle is not write mode.\n");
    return 0;//ShowLogHalt();
  }
  
  //progressStart(0);
  
  u32 SecPerClus=filesysSecPerClus;
  u32 blocksize=SecPerClus*512;
  u8 *pblockbuf=(u8*)malloc(blocksize);
  if(pblockbuf==NULL){
    iprintf("Fatal error: FAT2_SetSize block memory overflow.\n");
    return 0;//ShowLogHalt();
  }
  
  //MemSet32CPU((FillChar<<0)|(FillChar<<8)|(FillChar<<16)|(FillChar<<24),pblockbuf,blocksize);
  memset(pblockbuf, FillChar, blocksize);
  
  u32 ClusListMax=(size+blocksize-1)/blocksize;
  u32 *pClusList=(u32*)malloc(ClusListMax*4);
  if(pClusList==NULL){
    iprintf("Fatal error: FAT2_SetSize Cluster list memory overflow.\n");
    return 0;//ShowLogHalt();
  }
  
  //MemSet32CPU(0,pClusList,ClusListMax*4);
  memset(pClusList, 0, ClusListMax*4);
  
  u32 ClusListCount=0;
  
  {
    iprintf("FAT2_SetSize: Reserve cluster.\n");
    ...
    while(reqsize!=0){
      ...
      if((curClus==0)||(curClus==CLUSTER_FREE)){
        FAT_fclose(file);
        iprintf("Fatal error: FAT2_SetSize disk full.\n");
        return 0;//ShowLogHalt();
      }
      pClusList[ClusListCount++]=curClus;
      progressDraw(ClusListCount+1 ,ClusListMax*2);
  ...
  if(FillChar != 0xAB){
	u32 idx;
    iprintf("FAT2_SetSize: Initial data area.\n");
    
    extern LPIO_INTERFACE active_interface;
    
    for(idx=0;idx<ClusListCount;idx++){
      u32 curClus=pClusList[idx];
      if((curClus==0)||(curClus==CLUSTER_FREE)){
        FAT_fclose(file);
        iprintf("Fatal error: FAT2_SetSize Initial error.\n");
        return 0;//ShowLogHalt();
      }
...
int FAT_mkdir (const char* path)
{
	...
	{
		iprintf("Fatal error: Can not mkdir. Directory is exists.\n");
	...
	while (path[pathPos + filePos] != '\0')
	{
		if (path[pathPos + filePos] == '/')
		{
			...
			if (FAT_chdir(pathname) == false) 
			{
				...
				iprintf("Fatal error: Can not mkdir. Can not create directory.\n");
	...
	{
		iprintf("Fatal error: Can not tell. Couldn't get a new cluster for the directory.\n");
	...
	{
		iprintf("Fatal error: Can not tell. Couldn't add the directory entry.\n");
...
u32 FAT_getFileEntry(u32 cluster)
{
	return (filesysRootDir + (cluster - CLUSTER_FIRST)*filesysSecPerClus) * filesysBytePerSec;
}
